...
public class ContainerAdvancedRuneReader extends Container {


	private TileEntityAdvancedRuneReader te;

	public ContainerAdvancedRuneReader(IInventory playerInv, TileEntityAdvancedRuneReader te)
	{
		this.te = te;
		...
		for (int y = 0; y < 3; ++y) {
			for (int x = 0; x < 3; ++x) {
				this.addSlotToContainer(new RecipeShowSlot(te, x + y * 3, 26 + x * 18, 24 + y * 18));
			}
		}
		
		//Rune Slot, Slot 9
		this.addSlotToContainer(new AdvancedRuneInputSlot(te, 9, 80, 98, te.getWorld()));
		
		
		//Recipe output Slot, Slot 10
		this.addSlotToContainer(new RecipeShowSlot(te, 10, 120, 42));
